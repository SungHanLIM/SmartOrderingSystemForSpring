		...
		<div align="center">
			<div class="section-main">
				<div class="section-title" align="center">
					...
					<c:choose>
						<c:when test = "${fn:length(list) > 0}">
							<c:forEach items="${list}" var="row">
								...
								<a href="javascript:openModal(${row.M_IDX});" class="remodalBtn">자세히보기 </a>								<!-- 임 ${row.M_CAT_ID} 이걸로 카테고리 나눠야함 -->
							</c:forEach>
						</c:when>
						<c:otherwise>
							<tr>
					...
					function openModal(parsed_M_IDX){
						var M_IDX = parsed_M_IDX;
							M_IDX = M_IDX-1;
							
			            $.getJSON('${ctx}/mMenuModal.do', function(json) {
			            	...
			            	side_information = 
							"<br>" + listResult[M_IDX].S_NAME + "<br>" +
							imgTagStart + listResult[M_IDX].S_PICTURE + imgTagEnd;
			            	
		            		major_information =
		            		"<div class='mMain-Big-title' align='center'>"+
		            		listResult[M_IDX].M_NAME+"<br>" + 
		            		listResult[M_IDX].M_EN_NAME+"</div>"+"<hr>"+
							"조리시간 : " + listResult[M_IDX].M_COOK_TIME + "분<br>" +
        					"가격 : " + listResult[M_IDX].M_PRICE + "원<br>" + 
                            "재료 : " + listResult[M_IDX].M_COMPOSE + "<br>" + 
                            listResult[M_IDX].M_DETAIL + "<br>" + 
                            "<hr>" + "<div class='mMain-title' align='center'>" +
                            "SIDE MENU" + "</div> " + side_information;
